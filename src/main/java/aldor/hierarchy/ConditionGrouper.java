...
public class ConditionGrouper {
    ...
    List<SyntaxWithCondition> parents(Syntax syntax) {
        ...
        while (!candidates.isEmpty()) {
            ...
            List<SyntaxWithCondition> expanded = parents.stream().map(p -> p.is(If.class)
                    ...
                    : new SyntaxWithCondition(p, candidate.conditions())).toList();
            candidates.addAll(expanded);
        }
        return allParents;
    }

    private static boolean matches(Collection<SyntaxWithCondition> allParents, SyntaxWithCondition candidate) {
        return allParents.stream().anyMatch(p -> p.match(candidate));
    }

    List<SpadLibrary.Operation> operations(Iterable<SyntaxWithCondition> parents) {
    ...
    private static SyntaxWithCondition withCondition(Collection<Syntax> conditions, Syntax condition, Syntax thenPart) {
        return new SyntaxWithCondition(thenPart, Streams.concat(Stream.of(condition), conditions.stream()).collect(Collectors.toList()));
    }

    public static final class SyntaxWithCondition {
