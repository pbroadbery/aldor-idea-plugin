...
public class AldorFacetUtil {
    public static String getFacetLibraryName(AxiomInstalledSdk type, String name) {
        return name + type.librarySuffix();
    }

    public static void updateLibrary(Module module, AldorFacetConfiguration facetConfiguration, AxiomInstalledSdk sdkType) {
        ApplicationManager.getApplication().runWriteAction(() -> {
            final ModuleRootManager rootManager = ModuleRootManager.getInstance(module);
            final ModifiableRootModel model = rootManager.getModifiableModel();
            boolean modelChanged = false;
            final Sdk sdk = facetConfiguration.sdk();
            try {
    ...
    public static void removeLibrary(AxiomInstalledSdk sdkType, Module module) {
        ApplicationManager.getApplication().runWriteAction(() -> {
            final ModuleRootManager rootManager = ModuleRootManager.getInstance(module);
            final ModifiableRootModel model = rootManager.getModifiableModel();
